...
describe("API SERVER TEST V2 ROUTER: ", () => {
  ...
  it("Should create a record using POST", async () => {
    let res = await mockReq.post("/api/v2/food").send(foodTest);
  ...
  it("Should read a record using GET", async () => {
    let newFood = await mockReq.post("/api/v2/food").send(foodTest);
    let id = newFood.body._id;
    let res = await mockReq.get(`/api/v2/food/${id}`);
  ...
  it("Should read a list of record using GET", async () => {
    let res = await mockReq.get("/api/v2/food");
  ...
  it("Should update a record using PUT", async () => {
    ...
    let oldFood = await mockReq.post("/api/v2/food").send(foodTest);
    let id = oldFood.body._id;
    let res = await mockReq.put(`/api/v2/food/${id}`).send(newRecord);
    expect(res.status).toEqual(200);
  ...
  it("Should update a record using DELETE", async () => {
    let newFood = await mockReq.post("/api/v2/food").send(foodTest);
    let id = newFood.body._id;
    let res = await mockReq.delete(`/api/v2/food/${id}`);
    expect(res.status).toEqual(200);
    let getResponse = await mockReq.get(`/api/v2/food/${id}`);
